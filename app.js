...
async function initializeMap() {
    try {
        // Check if this is dual-panel layout
        const isDualPanel = document.getElementById('map-left') && document.getElementById('map-right');

        if (isDualPanel) {
            // Dual-panel layout: embed same map in both panels
            console.log('Initializing dual-panel Felt maps...');

            const feltLeft = await Felt.embed(
                document.getElementById('map-left'),
                'ZW3dAXAYSgKdErRegZUdHA',
                {
                    uiControls: {
            ...
            const feltRight = await Felt.embed(
            ...
            felt = feltLeft; // Use left map as primary
            console.log('Dual-panel maps loaded successfully');
        } else {
            // Single map layout
            const mapContainer = document.getElementById('map');
            if (!mapContainer) {
                console.error('Map container element not found');
                showError('Map container not found. Check page layout.');
                return;
            }
...
function showError(message) {
    ...
    if (mapContainer) {
        ...
        // Fallback: show alert if no map container
        alert('Error: ' + message);
    }
}
